...
![Dreamer imagining Mario: real gameplay, the model's own imagination, and prediction error, side by side](docs/assets/showcase.gif)

`[ real gameplay | the model's imagination | prediction error ]` — the model watches 8 real
frames, then predicts the next 25 purely from its learned latent dynamics (no more real
observations), decoded back to pixels. Gray in the error panel means a pixel-perfect prediction;
generated by `scripts/dream.py --horizon 25` against this project's own `trial` checkpoint (1M
Dreamer steps) — the default `--horizon 56` still works, but pushes well past the point where the
imagined panel stays sharp, so a shorter horizon reads better as a demo.
